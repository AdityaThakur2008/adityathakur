  ...
  return (
    <div className="group flex flex-col bg-card border border-border/60 hover:border-primary/40 rounded-3xl p-6 transition-all duration-300">
      <div className="flex items-start justify-between mb-5">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-lg ${project.logoBg}`}>
          {isIcon ? <Logo size={24} /> : project.logoIcon}
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {project.tags.map((tag: any, i: number) => {
          const Icon = tag.icon;
          return (
            <span key={i} className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-secondary/30 border border-border/40 text-[10px] font-medium text-muted-foreground">
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-border/50">
        ...
        >
          {project.links.demo ? 'Live Demo' : 'View Project'} <ArrowUpRight size={14} />
        </a>
        <a 
